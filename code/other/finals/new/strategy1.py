...
def cnn(rank, world, args):
    ...
    print('Rank: {}. Train loader: {}. Num_workers: {}'.format(rank, len(train_loader), n_w))

    num_epochs = args.epochs
    iter = 0

    if rank == 0:
        #df = pd.DataFrame({'num_workers': [0], 'num_nodes': [0], 'bs': [0], 'num_epochs': [0], 'acc': [0], 'time': [0]})
    ...
    if rank == 0:
        ...
        df = pd.DataFrame({'exp_number': [args.nw], \
                           'num_nodes': [world], \
                           'bs': [batch_size], \
    ...
    print('Rank: {}. Batch size: {}. Iteration: {}. Epochs: {}. Loss: {}. Accuracy: {}'.format(rank, batch_size,
                                                                                                       iter, num_epochs,
                                                                                                       loss.item(),
                                                                                                       accuracy))
